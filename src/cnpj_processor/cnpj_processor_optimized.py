...
class CNPJProcessorOptimized:
    ...
    def connect_database(self):
        ...
        try:
            # Remover connection_timeout se existir (não suportado pelo
            # pymysql)
    ...
    def get_total_count(self, filters_apply: Dict[str, Any] = None) -> int:
        """
        ...
        if filters_apply:
            query = self.apply_filters_apply_optimized(query, filters_apply, count_only=True)
    ...
    def apply_filters_apply_optimized(
        self, query: str, filters_apply: Dict[str, Any], count_only: bool = False
    ) -> str:
        """
        ...
        if "uf" in filters_apply:
            where_conditions.append(f"est.uf = '{filters_apply['uf']}'")

        # Filtro Situação Cadastral (muito comum)
        if "situacao_cadastral" in filters_apply:
            situacao = filters_apply["situacao_cadastral"]
            if situacao == "ativos":
        ...
        if "codigo_municipio" in filters_apply:
            where_conditions.append(
                f"est.codigo_municipio = {
                    filters_apply['codigo_municipio']}"
            )

        # Filtro CNAE Codes (usar IN para melhor performance)
        if "cnae_codes" in filters_apply:
            cnae_list = "','".join(filters_apply["cnae_codes"])
            where_conditions.append(f"est.cnae IN ('{cnae_list}')")

        # Filtro Data de Início de Atividade
        if "data_inicio_atividade" in filters_apply:
            data_filtro = filters_apply["data_inicio_atividade"]
            if "inicio" in data_filtro:
                where_conditions.append(
                    f"est.data_inicio_atividade >= '{
                        data_filtro['inicio']}'"
                )
        ...
        # Filtro Com Email
        if "com_email" in filters_apply:
            if filters_apply["com_email"]:
                where_conditions.append(
                    "est.correio_eletronico IS NOT NULL AND est.correio_eletronico != ''"
                )
            else:
                where_conditions.append(
                    "(est.correio_eletronico IS NULL OR est.correio_eletronico = '')"
                )

        # Filtro Com Telefone
        if "com_telefone" in filters_apply:
            if filters_apply["com_telefone"]:
                where_conditions.append(
        ...
        if "opcao_tributaria" in filters_apply:
            if filters_apply["opcao_tributaria"] == "mei":
                where_conditions.append("s.opcao_mei = 'S'")
            elif filters_apply["opcao_tributaria"] == "sem_mei":
                where_conditions.append("(s.opcao_mei IS NULL OR s.opcao_mei != 'S')")

        # Filtro Capital Social
        if "capital_social" in filters_apply:
            if filters_apply["capital_social"] == "10k":
                where_conditions.append("e.capital_social > 10000")
            elif filters_apply["capital_social"] == "50k":
                where_conditions.append("e.capital_social > 50000")
            elif filters_apply["capital_social"] == "100k":
    ...
    def build_optimized_query(
        self, limit: int = 0, offset: int = 0, filters_query: Dict[str, Any] = None
    ) -> str:
        """
        ...
        # Aplicar filtros
        if filters_query:
            query = self.apply_filters_apply_optimized(query, filters_query)

        # Ordenação por data de início de atividade descendente (mais recentes
        # primeiro)
    ...
    def get_lookup_data(
        self, table: str, key_field: str, value_field: str, keys: tuple
    ) -> Dict[str, str]:
        """
        ...
        query = f"SELECT {key_field}, {value_field} FROM {
            table} WHERE {key_field} IN ({placeholders})"
    ...
    def process_batch(self, batch_data: pd.DataFrame) -> pd.DataFrame:
        ...
        cnae_lookup = self.get_lookup_data(
            "cnpj_cnaes", "cnae", "descricao", tuple(cnpj_list)
        )
        municipio_lookup = self.get_lookup_data(
            "cnpj_municipios",
            "codigo",
            "municipio",
            tuple(batch_data["codigo_municipio"].unique()),
        )
        pais_lookup = self.get_lookup_data(
            "cnpj_paises", "codigo", "pais", tuple(batch_data["codigo_pais"].unique())
        )
    ...
    def process_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Criar CNPJ completo
        df["cnpj"] = (
            df["cnpj_part1"].astype(str)
            + df["cnpj_part2"].astype(str)
            + df["cnpj_part3"].astype(str)
        )
    ...
    def apply_data_processing(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        # Detectar celulares
        df["telefone1_celular"] = df.apply(
            self.detect_celular, axis=1, args=("telefone1_celular", "ddd_telefone_1")
        )
        df["telefone2_celular"] = df.apply(
            self.detect_celular, axis=1, args=("telefone2_celular", "ddd_telefone_2")
        )
    ...
    def save_to_csv_batch(
        self, df: pd.DataFrame, output_path: str, append: bool = False
    ):
        """
    ...
    def run_optimized(
        self, limit: int = 0, output_path: str = None, filters_apply: Dict[str, Any] = None
    ):
        """
        ...
        try:
            ...
            total_records = self.get_total_count(filters_apply)
            MAX_LIMIT = 200000

            logger.info("Total de registros a processar: %s", f"{total_records:,}")
            logger.info(
                "📊 Limite global máximo: %s registros (mais recentes por data de início)",
                f"{MAX_LIMIT:,}"
            )
            ...
            if output_path is None:
                output_path = os.path.join(
                    OUTPUT_CONFIG["output_dir"], "cnpj_empresas.csv"
                )
            ...
            logger.info(
                f"Iniciando processamento em lotes de {
                    self.batch_size:,} registros..."
            )
            ...
            while processed < total_records:
                ...
                query = self.build_optimized_query(
                    limit=current_batch_size, offset=processed, filters_query=filters_apply
                )
                ...
                logger.info(
                    f"Lote {batch_num}: {len(df_processed):,} registros processados "
            ...
            logger.info(
                f"Processamento concluído! {
                    processed:,    } registros salvos em: {output_path}"
            )
...
def run_optimized_processing(
    ...
    processor.run_optimized(limit=limit, output_path=output_path, filters_apply=filters_proc)


if __name__ == "__main__":
    # Exemplo de uso
    processor_example = CNPJProcessorOptimized()

    # Filtros de exemplo
    filters_apply = {"uf": "SP", "situacao_cadastral": "ativos", "com_email": True}

    processor_example.run_optimized(
        limit=100000, output_path="output/sp_ativos_com_email.csv", filters_apply=filters_apply
    )
